...
class API_BUILDER:
    ...
    def __init__(self, raw_content: List[Dict], num_samples: int, tokenizer: AutoTokenizer, 
                ...
                benchmark_type: str = "tool_calling", save_meta_data: bool = False) -> None:
    ...
    def calculate_api_bucket_nums(self, num_samples, api_type, api_pool_num):
        if api_type == "multiple_api":
            return num_samples + math.ceil(num_samples / (api_pool_num // 2))
        return num_samples + math.ceil(num_samples / api_pool_num)
    
    def build_api_benchmark(self, api_type='single_api', save_to_disk=False):
        for api_pool_num in self.api_pool_nums:
            # re-calculate the num_samples since some apis are treated as the demonstration
            cur_num_samples = self.calculate_api_bucket_nums(api_type, api_pool_num)
            bucket_pool = []
            ...
            for query_group in tqdm(query_bucket, unit="sample"):
                current_api_pool, _ = read_all_apis(query_group, api_type=api_type)
                ...
                if len(current_api_pool) >= api_pool_num + 1:
                    ...
                    continue
                remain_api_pool, _ = read_all_apis(other_api_pool, api_type=api_type)
            ...
            for b_id, query_group in enumerate(query_bucket):
                for sample in query_group:
                    tool_sample = ToolSample(sample, type=api_type, benchmark_type=self.benchmark_type, total_pool=bucket_pool[b_id])
        ...
        if save_to_disk:
            auto_save_data(processed_apis_json, f"{self.save_dir}/{api_type}/{self.benchmark_type}/num_{api_pool_num}_pool.json")
