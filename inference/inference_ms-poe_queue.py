...
# cd inference
# nohup python inference_ms-poe_queue.py >ms_poe_draft.log
...
class MsPoeWorker(mp.Process):
    ...
    @classmethod
    def process(cls,model, tokenizer, sample, results):
    ...
    def run(self):
        ...
        while True:
            sample=self.task_queue.get()
            if sample is None:break
            self.process(model,tokenizer,sample,self.results)
...
if __name__=="__main__":
    ...
    allocated_gpus=allocate_gpu(tp_size=2, gpu_list=[0,1,4,5])
    ...
    processes = []
    for i,visable_gpu in enumerate(allocated_gpus):
        print("Process: ",i)
        process=MsPoeWorker(visable_gpu=allocated_gpus[i],
                            args=args,
    ...
    prepare_babilong_data(tokenizer=tokenizer,task_queue=task_queue)
    
    for _ in range(len(allocated_gpus)):
        task_queue.put(None)
