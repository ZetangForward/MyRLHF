...
class LLMNeedleHaystackTester:
    ...
    def niah_evaluate_and_log(self, *args):
        self.evaluate_and_log(*args)

    def run_test(self, args):
        # Run through each iteration of context_lengths and depths
        if not self.implicit_reasoning:  # niah testing
        ...
        for layer_idx in range(self.layer_num):
            for head_idx in range(self.head_num):
                ...
                for v, i in zip(values, idx):
                    if self.implicit_reasoning: # reasoning head ?
                        if self.check_if_attend_ref(i, retrieval_score, layer_idx, head_idx):
                            break
                    else: # niah testing, retrieval head ?
                        if self.needle_pos[0][0] <= i < self.needle_pos[0][1] and inp.item()==self.prompt_ids[i].item():
                            retrieval_score[layer_idx][head_idx][0] += 1/(self.needle_pos[0][1] - self.needle_pos[0][0])
    ...
    def retrieval_head_accumulate(self, retrieval_score):
        for layer_idx in range(self.layer_num):
            for head_idx in range(self.head_num):
                ...
                # self.detected_tokens.extend(retrieval_score[layer_idx][head_idx][1])
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        if not self.implicit_reasoning:
            ...
            if self.model_version in ["Mistral-7B-Instruct-v0.2", "Qwen1.5-14B-Chat"]:
                ...
                input_ids = self.enc.apply_chat_template([{ "role": "user", "content": input_context}], tokenize=True, add_generation_prompt=True, return_tensors='pt')
                # input_ids = self.enc(input_context , return_tensors="pt")['input_ids']
        else:
    ...
    def start_test(self, args):
        for ni in range(len(self.needle_list)):
            ...
            self.golden_answer = self.golden_answer[ni] if self.implicit_reasoning else None
            if self.print_ongoing_status:
        ...
        with open(f"head_score/{self.tag}/{self.model_version}.json", 'w') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--multi_hop_reasoning', action="store_true", help='whether is multi-hop reasoning task')
    ...
    if multi_hop_reasoning:
        print("Multi-hop reasoning task")
    else:
        print("Needle in a haystack task")
