...
class LanguageModelingDataset(Dataset):
    ...
    def __init__(
        ...
        if num_training_samples > 0:
            processed_dataset = processed_dataset.select(range(num_training_samples))
    ...
    def process_data(self, data):
        ...
        # avoid prompt length shorter than max length (specifical for pretrain mode)
        prompt_token = self.tokenizer(
            prompt,
            max_length=self.max_length,
            padding=False,
        ...
        if prompt_token["attention_mask"].int().sum().item() < self.max_length:
            prompt = None
